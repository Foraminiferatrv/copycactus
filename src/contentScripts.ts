...
function copyToTheClipboard(textToCopy: string) {
...
function executeWhenPageMatches(path: string, fn: () => any) {
...
function sendLidData(target: HTMLElement): void {
  ...
  const lidPositionElement = parentElement.querySelector('div.artdeco-entity-lockup__subtitle>span') as HTMLAnchorElement
...
function appendCopybutton(elementList: HTMLElement[]): void {
  elementList.forEach((element: any, i: number) => {
    let copyButtonClone = copyButton.cloneNode(true) as HTMLElement
    copyButtonClone.setAttribute('id', 'copycactus-button' + i)
    copyButtonClone.onclick = (e) => sendLidData(e.target as HTMLElement)
